# Jupyter Exploration

## Course Assignment
A01 ITAI 1378 – Colab and Other Tools Practice Run

## Activity Description
This project is part of a **Setting Up and Exploring GitHub (20 minutes)** exercise.  
It includes practicing the use of GitHub, Google Colab, and other introductory data science tools.

## Objectives
- Set up a GitHub repository
- Learn basic Markdown formatting
- Practice using Google Colab and related tools
- Understand how to document work in a README file

## Tools Used
- GitHub
- Google Colab
- Jupyter Notebook (if applicable)

## Notes
This repository is for practice and learning purposes as part of an introductory ITAI 1378 assignment.
